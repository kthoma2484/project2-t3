...
$(function () {
        ...
        $.get(queryURL).then(function (nameData) { //first query (getting origin info)
            ...
            $.get(queryURL2).then(function (relatedNames) { //new query using last names origin
                ...
                if (opt2NumNames === "1") {
                    ...
                    displayResult();
                    //   console.log(`Welcome baby ${relatedNames.names[0]} ${opt2LastName} `); //OPT2 RESULT
                }
                if (opt2NumNames === "2") {
                    $('#result-div').append(`<h1>Your name is: ${country}</h1> <h1> Welcome baby ${relatedNames.names[0]} ${relatedNames.names[1]} ${opt2LastName}</h1> `)
                    displayResult();
                    //   console.log(`Welcome baby ${relatedNames.names[0]} ${relatedNames.names[1]} ${opt2LastName} `); //OPT2 RESULT
                }
                if (opt2NumNames === "3") {
                    $('#result-div').append(`<h1>Your name is: ${country}</h1> <h1> Welcome baby ${relatedNames.names[0]} ${relatedNames.names[1]} ${relatedNames.names[2]} ${opt2LastName}</h1> `)
                    displayResult();
                    // console.log(`Welcome baby ${relatedNames.names[0]} ${relatedNames.names[1]} ${relatedNames.names[2]} ${opt2LastName} `); //OPT2 RESULT
                }
            });
        });
    });
    ...
    let totalResults = 20;
    $('#result-div').append(`<div id="names-table></div> `); //create grid container
    $('#result-div').append(`<table id="namesTable">
    <tr>
        <th>
            <h2><i class="far fa-question-circle"></i> Name</h2>
        </th>
        <th>
            <h2><i class="fas fa-venus-mars"></i> Gender</h2>
        </th>
        <th>
            <h2><i class="far fa-calendar-alt"></i> Year</h2>
        </th>
        <th>
            <h2><i class="fab fa-earlybirds"></i> Count</h2>
        </th>
    </tr>
</table>`);  //CREATE HEADER
    if(data.length < totalResults) i = data.lenth;
    // $('#result-div').append(`<h1>Your name is: ${country}</h1> <h1> Welcome baby ${relatedNames.names[0]} ${opt2LastName}</h1> `)
    $('#result-div').append(`<table id="namesGrid"></table>`)
    for(i = 0; i < totalResults; i++){ //for each object in data
        $('#namesGrid').append(`<tr>
        <td>${data[i].name}</td>
            <td>${data[i].gender}</td>
                <td>${data[i].year}</td>
    ...
    //     $('#result-div').append(`<div class="row">
    //     <div class="col-3">
    //       ${data[i].name}
    //     </div>
    //     <div class="col-1">
    ...
    //   </div>`); //Create new row per element
    };
    displayResult();
